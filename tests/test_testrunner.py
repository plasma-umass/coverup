...
@pytest.mark.skip(reason="Temporarily disabled due to cleanslate problems")
...
# fail_collect temporarily disabled because of issues in pytest-cleanslate
#@pytest.mark.parametrize("fail_collect", [True, False])
@pytest.mark.parametrize("fail_collect", [False])
@pytest.mark.parametrize("absolute", [True, False])
def test_measure_suite_coverage_isolated(absolute, fail_collect):
    with tempfile.TemporaryDirectory(dir=Path('.')) as tests_dir:
        ...
        # the isolation should prevent the tests from failing
